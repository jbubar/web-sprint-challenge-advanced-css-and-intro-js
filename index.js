...
function heading(heading_title){
  console.log(`\n\n\t********      ${heading_title}      ********\n\n`);
}
// 🖌🖼 M V P 🖼🖌 //
...
function get20s(artists){
  ...
  for(let i = 0; i < artists.length; i++){
    if(artists[i].years >= "1900" && artists[i].years <= "2000" || artists[i].years[7] == "1" && artists[i].years[8] == "9"){
      artists1900s+= artists[i].name + ", ";
    }
  }
  return artists1900s;
}
...
heading("Task 5");
console.log(`Before using the function removeArtist() the length of the artists array is ${artists.length},\n and after calling the function, the length of the array is:`);
